...
function renderFitness(fit) {
  ...
  el.innerHTML = `<div class="fitness-bar">
    ...
    <div class="fitness-explain">
      ${fit.hurst < 0.45 ? '✓ Hurst < 0.45 — mean-reverting. Strategy edge is real.'
        : fit.hurst < 0.50 ? '~ Hurst near 0.5 — weak mean-reversion. Expect modest returns.'
        : '✗ Hurst > 0.5 — trending pair. Rebalancing will underperform hold.'}
      ${fit.ratioDrift > 15 ? ` Ratio drifted ${fit.ratioDrift}% — consider wider band.` : ''}
    </div>
  </div>`;
  ...
  try {
    ...
    if (res.error) {
      ...
      setStatus(r.vsHold >= 0 ? 'ok' : 'warn',
        ...
        `  ·  max swap ${state.perf.maxSizePct}% of pool  ·  ${state.perf.thrashCount === 0 ? 'no thrashing' : '⚠ thrashing detected'}`);
    }
...
function renderDiagnostics(p, r) {
  ...
  $('diagnosticsPanel').innerHTML = `
    <div class="diag-grid">
      ...
      <div class="diag-box">
        <div class="diag-title">Trading Edge (Point 1)</div>
        <div class="diag-row"><span>Gross P&amp;L</span>
          <strong class="${p.grossTotal >= 0 ? 'pos' : 'neg'}">${p.grossTotal>=0?'+':''}${inr(p.grossTotal)}</strong></div>
        <div class="diag-row"><span>Brokerage paid</span><strong class="neg">−${inr(p.brokTotal)}</strong></div>
        <div class="diag-row"><span>Net cash</span>
          <strong class="${p.netTotal >= 0 ? 'pos' : 'neg'}">${p.netTotal>=0?'+':''}${inr(p.netTotal)}</strong></div>
        <div class="diag-row"><span>Friction ratio</span><strong>${dec(p.frictionPct, 1)}% of gross</strong></div>
        <div class="diag-note">${p.narrative.friction}</div>
      </div>

      <div class="diag-box">
        <div class="diag-title">Vault — Realised Profit (Point 5)</div>
        ...
        <div class="diag-row"><span>IL status</span>
          <strong class="${r.ilPct >= 0 ? 'pos' : 'neg'}">${pct(r.ilPct, 2)}</strong></div>
        <div class="diag-row"><span>IL in ₹</span>
          <strong class="${r.ilINR >= 0 ? 'pos' : 'neg'}">${r.ilINR>=0?'+':''}${inr(r.ilINR)}</strong></div>
        <div class="diag-note">Vault shares locked — never re-exposed to IL or swaps.</div>
      </div>

      <div class="diag-box">
        <div class="diag-title">Trade Quality (Point 4)</div>
...
function renderStats(r, a1, a2) {
  const cells = [
    ...
    { label:'Band width',                        value: `±${dec(r.bandPct,1)}%`,  cls:'' },
    { label:'Concentration',                     value: `${r.concentration}×`,    cls:'' },
    { label:'Initial L',                         value: r.initialL ? r.initialL.toLocaleString('en-IN',{maximumFractionDigits:0}) : '—', cls:'' },
    ...
    { label:`Pool ${a1} shares`,                value: `${qty(r.holdX)} → ${qty(r.poolX)}`, cls:'' },
    { label:`Pool ${a2} shares`,                value: `${qty(r.holdY)} → ${qty(r.poolY)}`, cls:'' },
...
function renderCharts() {
  ...
  $('legend1').innerHTML = `
    ...
    <div class="leg"><div class="leg-line" style="background:#444"></div>Buy &amp; Hold</div>
  ...
  [
    ...
    ro.observe(canvas.parentElement); _obs.push(ro);
  });
}
  ...
  const html = rows.map(row => {
    if (row.type === 'VAULT_DEPOSIT') return `
      <tr class="row-vault">
        ...
        <td colspan="3">
          ...
          · IL-split: <strong>${row.ilCorrecting}</strong>
        </td>
    ...
    if (row.type === 'ADJUST') return `
      <tr class="row-adjust">
        ...
        <td colspan="3">
          Ratio ${dec(row.rNow, 4)} → target [${dec(row.rLow,4)}, ${dec(row.rHigh,4)}]
          · L recalculated: ${row.L}
        </td>
        <td class="r">—</td>
...
function exportCsv(ledger) {
  const h = ['Date','Type','Action',
             'SellQty','SellProceeds_INR','BuyQty','BuyCost_INR',
             'Gross_INR','Brokerage_INR','Net_INR','Cash_INR',
             'PoolA1','PoolA2','VaultA1','VaultA2',
             'IL_Pct','Ratio','L'];
  const lines = [h.join(',')].concat(ledger.map(r => {
    if (r.type === 'VAULT_DEPOSIT') return [
      r.date,'VAULT_DEPOSIT','"'+r.action+'"',
      '','',r.buyX+'+'+r.buyY,'',
    ...
    if (r.type === 'ADJUST') return [
      ...
      '',''+r.rNow.toFixed(6),r.L
    ].join(',');
    return [
      r.date,'TRADE','"'+r.action+'"',
      r.sellQty, r.sellProceeds.toFixed(2),
      r.buyQty,  r.buyCost.toFixed(2),
      r.gross.toFixed(2), r.brok.toFixed(2), r.net.toFixed(2), r.cashProfit.toFixed(2),
      r.poolX, r.poolY, r.vaultX, r.vaultY,
      r.ilPct.toFixed(4), r.rNow.toFixed(6), r.L
    ].join(',');
  }));
  const blob = new Blob([lines.join('\n')], { type: 'text/csv' });
  const url  = URL.createObjectURL(blob);
  Object.assign(document.createElement('a'), { href: url, download: 'institutional_v8_ledger.csv' }).click();
